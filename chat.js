import Anthropic from "@anthropic-ai/sdk";

const KNOWLEDGE_BASE = `
=== 2025년 프리팁스(Pre-TIPS) 및 포스트팁스(Post-TIPS) 창업기업 지원계획 통합공고 ===

【사업 개요】
- 사업목적: 민간투자와 연계하여 유망 창업기업 발굴, 글로벌 유니콘 기업으로 육성
- 선정규모: 프리팁스 50개사 내외, 포스트팁스 77개사 내외
- 공고: 중소벤처기업부 공고 제2025-297호 (2025년 5월 8일)

【접수 일정】
- 1차: 2025년 5월 8일(목) ~ 5월 27일(화), 16:00까지
- 2차: 2025년 8월 1일(금) ~ 8월 20일(수), 16:00까지
- 접수 방법: K-Startup 누리집 온라인 신청 (www.k-startup.go.kr)

■ 프리팁스(Pre-TIPS)

【지원 대상】
- 창조경제혁신센터가 5천만원 이상 투자유치 및 추천한 비수도권 소재 초기창업기업
- 창업 후 3년 이내 기업 (2022년 5월 9일 ~ 2025년 5월 8일)
- 비수도권: 서울, 경기, 인천 제외

【지원 내용】
- 지원기간: 8개월
- 정부지원사업비: 최대 1억원 (평균 91백만원)
- 자기부담사업비: 총 사업비의 30% 이상 (현금 10% 이상, 현물 20% 이하)

【신청 자격 요건】
- 요건①: 모집공고일 기준 1년 이내 창조경제혁신센터가 5천만원 이상 투자 및 추천한 기업
- 요건②: 본사 또는 공장이 비수도권 소재 기업
- 요건③: 대표자 포함 상시 종업원 수 2인 이상

【제출 서류】
① 사업계획서 (PDF), ② 신청 기본사항, ③ 참가신청서, ④ 개인정보 동의
⑤ 투자계약서·투자금 납입내역, ⑥ 법인등기부등본 및 사업자등록증
⑦ 창조경제혁신센터 추천 확인서, ⑧ 투자사실확인서, ⑨ 청렴서약서
⑩ 주주명부, ⑪ 4대 사회보험 사업장 가입 명부, ⑫ 비수도권 소재 확약서
⑬ 사실증명, ⑭ 기타 가점 및 성과 관련 증빙서류
- 파일 용량 제한: 최대 30MB (압축파일 첨부 가능)

【평가 절차】
- 1차: 신청(5.8~5.27) → 요건검토(6월) → 발표평가(6월 말) → 최종선정(7월 초)
- 2차: 신청(8.1~8.20) → 요건검토(8월 말) → 발표평가(9월 초) → 최종선정(9월 중)
- 발표평가: 40분 이내 (발표 20분 + 질의응답 20분), 60점 미만 시 탈락

【가점 항목 (최대 2점)】
- 스타트업 10대 초격차 분야: 1점
- 내일채움공제(플러스) 도입·운영: 1점

【투자 인정 기준】
- 단독투자: 창조경제혁신센터 5천만원 이상, 공고일 기준 1년 이내
- 공동투자: 창조경제혁신센터 50% 이상 투자 의무
- 신주투자만 인정, 전환사채(CB)·조건부지분전환계약(CN) 불가

■ 포스트팁스(Post-TIPS)

【지원 대상】
- 팁스 R&D 수행 후 "완료(성공)" 판정받은 창업기업
- 창업 후 7년 이내 (신산업 창업분야는 10년 이내)
- 후속투자 유치 금액 100억원 미만

【지원 내용】
- 지원기간: 18개월
- 일반: 정부지원사업비 최대 5억원, 자기부담 30% 이상
- 최우수: 정부지원사업비 최대 7억원, 자기부담 20% 이상
- 평균 지원액: 3.3억원

【최우수 기업 기준】
- 팁스 R&D 졸업평가에서 성공 요건의 2배 초과 달성
- 별도 신청 없이 자동 검토
- 최우수 기업이라도 선정평가 결과에 따라 차등 배정 (모두 7억원 보장 아님)

【신청 자격 요건】
- 요건①: 팁스 R&D '후속투자 유치' 성공조건 달성
- 요건②: 후속투자 외 팁스 R&D 성공조건 달성 + 민간 후속투자 10억원 이상

【팁스 R&D 성공 기준 (요건② - 5개 중 1개 이상)】
① M&A 성사 (10억원 이상), ② 기업공개 (IPO, 코넥스 포함)
③ 연간 매출액 10억원 이상, ④ 연간 수출액 50만불 이상
⑤ 신규고용 20명 이상

【주관기관】
- 한국벤처캐피탈협회: 02-3017-7026 / 02-3017-7029/7030/7032
- 한국기술벤처재단 (키스트이노베이션): 02-958-6695

【신청 권한 부여 방법】
- K-스타트업 아이디(대표자)를 주관기관에 제출 → 1~2일 후 권한 부여

【제출 서류 (포스트팁스)】
① 포스트팁스 사업계획서 (동의서 포함), ② 신청 기본사항, ③ 참가신청서
④ 개인정보 동의, ⑤ 투자계약서·투자금 납입내역, ⑥ 주주명부
⑦ 법인등기부등본(말소사항 포함) 및 사업자등록증, 대표자 신분증
⑧ 팁스 졸업 성공 공문 (협약서, 최종 보고서 포함)
⑨ 대표자 총사업자등록내역 사실증명
⑩ 사업자등록증명원 (다른 '계속' 사업자 보유 시)
⑪ 휴·폐업사업사실증명원, ⑫ 가점 및 성과 관련 증빙서류
- 파일 용량 제한: 30MB

【투자 인정 기준 (포스트팁스)】
- 인정: 창업기획자, 창업벤처투자, 개인투자조합 등의 보통주, 우선주(CPS, RCPS 등), SAFE
- 불인정: 개인투자(전문인력 아닌 경우), 메자닌 투자(CB, BW 등), 구주 매각, 투자조건부융자
- 2차 투자 인정 기한: 2025년 8월 20일까지 납입 완료
- 증빙: 투자계약서 + 송금확인증 + 납입보관증 모두 제출 필요

【평가 절차 (포스트팁스)】
- 1차: 신청(5.8~5.27) → 요건검토(6월) → 발표평가(6월 말) → 최종선정(7월 초)
- 2차: 신청(8.1~8.20) → 요건검토(9월 초) → 발표평가(9월 중) → 최종선정(9월 말)

【가점 항목 (최대 3점)】
- 청년창업기업 (대표자 만 39세 이하): 1점
- 스타트업 10대 초격차 분야: 1점
- 내일채움공제(플러스) 도입·운영: 1점

【사용 가능 비목】
- 재료비, 용역비, 인건비 등 (창업지원사업통합관리지침 제33조)

【팁스 R&D 아이템 연관성】
- 동일하지 않아도 되나 연관성이 크거나 고도화된 아이템으로 신청 가능

■ 공통 사항

【신청 제외 대상】
- 채무불이행 규제 중인 자, 국세·지방세 체납 중인 자
- 창업진흥원 환수금 반환 미종결 자
- 지원 제외 업종 (일반유흥주점업, 무도유흥주점업, 기타 사행시설)
- 2025년 창업사업화 지원사업 수행 중인 자
- 창업지원사업 참여 제한 중인 자, 휴·폐업 중인 자, 체불사업주

【2025년 동시수행 불가 사업】
예비창업패키지, 초기창업패키지, 창업도약패키지, 초격차 스타트업 1000+, TIPS 사업화,
특화창업패키지, 창업중심대학, 재도전성공패키지, 신사업창업사관학교, 청년창업사관학교,
글로벌창업사관학교, 글로벌 기업 협업 프로그램, 딥테크 밸류업 프로그램 등 총 27개 사업

【문의처】
- 프리팁스 전문기관: 중소기업 통합 콜센터 1357
- 포스트팁스 한국벤처캐피탈협회: 02-3017-7026
- 포스트팁스 한국기술벤처재단: 02-958-6695
- K-스타트업 누리집: www.k-startup.go.kr
`;

const client = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY,
});

export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }

  const { messages } = req.body;

  try {
    const response = await client.messages.create({
      model: "claude-sonnet-4-20250514",
      max_tokens: 1024,
      system: `당신은 2025년 프리팁스 및 포스트팁스 창업기업 지원사업 전문 안내 챗봇입니다.

아래 공고 내용을 기반으로만 답변하세요. 공고에 없는 내용은 절대 추측하거나 임의로 답변하지 마세요.

공고 내용에 없는 질문에는 반드시 다음 문구로 답변하세요:
"해당 내용은 담당자에게 직접 문의하시길 바랍니다."

답변 시 유의사항:
- 공고 내용에 있는 정보만 정확히 안내하세요
- 친절하고 명확하게 답변하세요
- 필요시 관련 연락처(한국벤처캐피탈협회 02-3017-7026, 한국기술벤처재단 02-958-6695, 통합콜센터 1357)를 안내하세요

=== 공고 내용 ===
${KNOWLEDGE_BASE}`,
      messages: messages,
    });

    const text = response.content[0].text;
    res.status(200).json({ message: text });
  } catch (error) {
    console.error("API Error:", error);
    res.status(500).json({ error: "API 호출 중 오류가 발생했습니다." });
  }
}
