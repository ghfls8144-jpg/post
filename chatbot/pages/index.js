...
const TAGS = [
  ...
  ['❌ 제외대상', '신청 제외 대상 13가지를 알려주세요'],
  ['🏢 주관기관', '주관기관은 어디이고 차이가 무엇인가요?'],
  ['🎯 창업프로그램', '창업프로그램은 어떤 내용인가요?'],
  ['🔬 신산업분야', '신산업 창업 분야 27가지를 알려주세요'],
  ['📊 사업비비목', '사업비 구성 비목을 알려주세요'],
  ['🚫 동시수행불가', '동시수행 불가 사업 목록을 알려주세요'],
  ['⚠️ 유의사항', '선정 후 유의사항을 알려주세요'],
  ['🏦 세정지원', '국세청 세정지원 내용을 알려주세요'],
  ['📞 문의처', '문의 연락처를 알려주세요'],
];
  ...
  return (
    <>
        ...
        <div className="chat">
          {msgs.length === 0 && (
            <div className="welcome">
              ...
              <p>2026년 포스트 팁스(Post-TIPS) 창업기업 모집공고 전체 내용을 학습했습니다.<br />신청 자격, 지원 금액, 절차 등 무엇이든 질문해 주세요.</p>
              <div className="notice">⚠️ 본 봇은 공고문 기반 참고용입니다. 최종 확인은 반드시 주관기관에 문의하세요.</div>
              <div className="qbtns">
                {[
                  ['신청 자격 확인', '내가 신청 가능한지 자격 요건을 알려주세요'],
                  ['지원금 상세', '사업화 자금은 얼마이고 어떻게 구성되나요?'],
                  ['제출 서류 안내', '신청 마감일과 제출 서류를 알려주세요'],
                  ['평가지표 확인', '평가지표 4가지를 자세히 알려주세요'],
                  ['문의처 안내', '문의 연락처를 알려주세요'],
                ].map(([l, q]) => (
      ...
      <style jsx>{`
        ...
        header { display: flex; align-items: center; gap: 12px; padding: 14px 20px; background: rgba(7,9,15,0.97); border-bottom: 1px solid #1e2535; flex-shrink: 0; }
        ...
        .title { font-size: 14px; font-weight: 700; background: linear-gradient(90deg,#fff,#93c5fd); -webkit-background-clip: text; -webkit-text-fill-color: transparent; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .bubble { padding: 12px 15px; border-radius: 12px; font-size: 14px; line-height: 1.8; word-break: keep-all; }
        ...
        .bubble :global(.hd) { display: block; color: #60a5fa; font-weight: 700; margin: 6px 0 3px; }
        ...
        .inputarea { padding: 14px 20px 18px; background: rgba(7,9,15,0.97); border-top: 1px solid #1e2535; flex-shrink: 0; }
